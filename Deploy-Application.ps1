...
Try {
	...
	[string]$appVendor = 'VelocityEHS'
	[string]$appName = 'MSDSOnline'
	[string]$appVersion = '1.0.2'
	[string]$appArch = 'x86'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.2'
	[string]$appScriptDate = '2022.07.25'
	...
	$64bitAppInstallName = "MSDSOnline"
	# Application install Path.
	$64bitAppInstallPath = "${Env:ProgramFiles}\Tuolumne County\MSDSOnline"
	# Application  install parameters.
	...
	$64bitAppUninstallName = "MSDSOnline"
	...
	[string[]]$appShortcutDirs = @("${Env:ProgramFiles}\Tuolumne County\MSDSOnline", "${Env:ProgramData}\Microsoft\Windows\Start Menu\Programs", "${Env:PUBLIC}\Desktop")
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Try {
			...
			Copy-Item -Path "$dirFiles\$64bitAppInstallName" -Destination "$64bitAppInstallPath" -Force -Recurse
		...
		Foreach ($appShortcutDir in $appShortcutDirs) {
			Try {
				...
				New-Shortcut -Path "$appShortcutDir\$64bitAppInstallName.lnk" -TargetPath "CONBFIDENTIAL" -IconLocation "$envProgramFiles\Tuolumne County\MSDSOnline\Assets\MSDSOnline_Icon.ico" -WindowStyle "Maximized" -Description "MSDSOnline"
			}
